...
export default function DealerPortal({ user, token, onLogout, onUserUpdate, currentLanguage, onLanguageChange }: DealerPortalProps) {
  const t = translations[currentLanguage];

  const [coordinates, setCoordinates] = useState<LocationCoordinates>(
    user.location || { lat: 20.5937, lng: 78.9629, address: 'Bulk Trading Hub' }
  );
  ...
  const [radius, setRadius] = useState<number>(20); // Default 20km for dealers
  ...
  useEffect(() => {
    const resolveAddress = async () => {
      ...
      if (coordinates.address && 
          coordinates.address !== 'Bulk Trading Hub' && 
  ...
  const handleDetectLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          alert('GPS permission blocked. Please enable geolocation to locate bulk sellers nearby.');
        }
      );
    }
  };
  ...
  return (
    <div className="bg-slate-950 min-h-screen pb-16 font-sans text-slate-100 selection:bg-blue-500 selection:text-black">
      {/* Top Banner */}
      <header className="bg-slate-900/80 backdrop-blur-md border-b border-slate-800/60 px-6 py-4 rounded-b-2xl sticky top-0 z-40 shadow-xl">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center space-x-3">
            <div className="bg-gradient-to-br from-blue-500 to-indigo-600 text-black p-2.5 rounded-xl shadow-inner">
              <ShoppingBag className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-xl font-extrabold text-white font-display">SmartKisan 360</span>
                <span className="bg-blue-950/60 text-blue-400 text-[10px] px-2.5 py-0.5 font-bold rounded-full uppercase tracking-wider border border-blue-900/40">
                  {t.dealerPortal}
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5">{t.welcome}, <strong className="text-white">{user.name}</strong> • APMC Licensee Active</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 w-full md:w-auto justify-end">
            ...
            <div className="flex items-center space-x-1.5 bg-slate-950 border border-slate-800 rounded-xl px-2.5 py-2">
              ...
              <select
                ...
                className="bg-transparent text-xs font-extrabold text-blue-400 outline-none border-0 p-0 pr-6 cursor-pointer focus:ring-0 animate-fade-in"
              >
            ...
            <button
              ...
              className="bg-blue-950/80 hover:bg-blue-900/40 text-blue-300 text-xs px-3.5 py-2.5 rounded-xl font-bold transition flex items-center space-x-1.5 border border-blue-900/30 cursor-pointer"
            >
      ...
      <div className="bg-slate-900 border-b border-slate-800/50 py-3 px-6 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3 text-xs">
          <div className="flex items-center space-x-2.5 text-slate-300 w-full md:w-auto">
            <div className="bg-blue-950/80 p-1.5 rounded-lg border border-blue-900/40 shrink-0">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl space-y-4">
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                className="w-full accent-blue-500 h-1.5 bg-slate-950 rounded-lg cursor-pointer border border-slate-800"
              />
              <p className="text-[10px] text-slate-500 leading-normal font-mono">Harness Haversine math to find bulk crops planted within {radius}km of your procurement hub.</p>
            </div>
          ...
          <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center space-x-2 font-display">
              <MapPin className="w-4 h-4 text-blue-400" />
              <span>Bulk Crop Geolocation Pins</span>
            </h3>
            ...
            <div className="h-48 rounded-xl overflow-hidden border border-slate-800 relative shadow-inner z-10">
              <LeafletMap
                ...
                mainMarkerColor="#2563eb"
                markers={filteredListings.map((item) => ({
                  id: item.id,
                  lat: item.location.lat,
                  lng: item.location.lng,
                  label: `${item.cropName} - ₹${item.priceForDealer ?? item.price}/kg (${item.quantity}kg)`,
                  color: '#16a34a',
        ...
        {/* RIGHT COLUMN: BULK DEALER DIRECT ACTIVE OFFERS */}
          ...
          <div className="bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-xl space-y-6">
            <div className="border-b border-slate-800/80 pb-4">
              <h2 className="text-xl font-extrabold text-white tracking-tight font-display">
                Bulk Agri-Products Listed Nearby
              </h2>
                  ...
                  >
                    <div className="space-y-3.5">
                      <div className="flex justify-between items-start">
                        <div>
                          <span className="text-[10px] bg-blue-950 text-blue-400 px-2.5 py-0.5 rounded-md font-bold border border-blue-900/30">
                            {item.cropName}
                          </span>
                          <h4 className="text-base font-bold text-white mt-2 font-display">{item.cropName} Bulk Harvest</h4>
                        </div>
                        <div className="text-right">
                          <span className="text-xl font-black text-blue-400 block font-display">₹{item.priceForDealer ?? item.price} <span className="text-xs font-normal text-slate-500">/ kg</span></span>
                      ...
                      <div className="space-y-1.5 text-xs text-slate-400 border-t border-slate-850 pt-3">
                        ...
                        <div className="flex items-center space-x-1.5">
                          ...
                          <span>Farming location is <strong>{item.distance} km</strong> away</span>
                        </div>
                        {item.description && (
                    ...
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-400 hover:to-indigo-500 text-black font-extrabold py-2.5 rounded-xl text-xs transition cursor-pointer"
                    >
                      Connect & Secure Bulk Deal
                    </button>
                  </div>
                ))}
              </div>
        ...
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 p-6 md:p-8 rounded-3xl max-w-md w-full border border-slate-800 shadow-2xl relative">
            <h3 className="text-lg font-bold text-white border-b border-slate-800 pb-3 mb-4 flex items-center space-x-2 font-display">
              ...
              <span>Bulk Deal Connection Protocol</span>
            </h3>

            <p className="text-xs text-slate-400 leading-relaxed mb-4">
              Compliant with APMC bulk procurement trade regulations. Call the verified grower to schedule freight collection:
            </p>

            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2 mb-6 text-xs">
              <div className="flex justify-between font-semibold text-slate-300">
                <span>Crop:</span>
                <span className="text-blue-400 font-bold">{selectedListing.cropName}</span>
              </div>
              <div className="flex justify-between font-semibold text-slate-300">
              ...
              <div className="flex justify-between font-semibold text-slate-300">
                ...
                <span className="text-white font-bold">{selectedListing.quantity} kg</span>
              </div>
              <div className="flex justify-between font-semibold text-slate-300">
                <span>Price per kg:</span>
                <span className="text-white">₹{selectedListing.priceForDealer ?? selectedListing.price} / kg</span>
              </div>
              <div className="flex justify-between font-semibold text-slate-300">
                <span>Distance:</span>
            ...
            <div className="space-y-2.5 mb-6">
              ...
              <a
                ...
                className="bg-blue-950/40 hover:bg-blue-950/80 text-blue-300 py-3.5 px-4 rounded-xl border border-blue-900/30 font-mono font-bold text-center text-lg tracking-widest flex items-center justify-center space-x-2 transition"
              >
